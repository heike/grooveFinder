...
`grooveFinder` makes use of the `imager` package, which stores images in `Cimg` format. 
...
Now that we have a two dimensional image representation of our bullet land scan, we create an image gradient, which detects changes in direction, intensity, or shading of pixels in our image. Using this gradient, we select only values above the 99th percentile, removing detail from our image in preparation for the Hough transform. XXX Are we still doing this? I thought we got rid of the quantile thresholding. Was that only after the Hough transform? XXX
...
XXX If we're not doing Canny Edge detection (as discussed above in the preprocessing stage) then this section needs to go (I've commented it out). It's just confusing - first we say we're not doing Canny Edge detection, then we talk about it. Focus on the application of the Hough transform to the data. XXX
...
XXX Move the theoretical Hough explanation into this section XXX


## Applying Hough Transform

Using the image gradient calculated during the preprocessing step, we begin the Hough transform process using the `hough_lines` function from `imager`^[Other implementations of Hough transforms in R include XXXX]. XXX Please expand on this - what representation does `Cimg` use (e.g. where is the origin), and what representation do we need for compatibility with `bulletxtrctr` and `x3ptools`? Pictures may be useful... XXX Because we are working in images that start at the origin we need to shift the hough transform results XXX this next bit doesn't seem to be connected to the shift mentioned earlier in the sentence. Try to connect it better by elaborating a bit? XXX and return a dataframe for line selection processes. We then transform the $theta$ values to be between XXX between...? XXX
