...
# Get Hough Groove Locations, this may take some time
bullets <- bullets %>%
  mutate(
    hough_fit = ccdata %>% purrr::map(.f = x3p_to_df) %>%
...
bullets <- bullets %>%
  mutate(
    hough_left = purrr::map(1:nrow(bullets), function(x){bullets$hough_fit[[x]]$left.groove.fit(crosscut[x])}),
    hough_right = purrr::map(1:nrow(bullets),
function(x){bullets$hough_fit[[x]]$right.groove.fit(crosscut[x])})
  )
...
bullets <- bullets %>%
  select(-hough_fit) %>%
  select(1:9, "grooves_hough", everything())
```
